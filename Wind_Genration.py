...
# ===================== KAFKA SETTINGS =====================
KAFKA_BOOTSTRAP_SERVERS = ['localhost:19092']  # Update with your Kafka broker address
KAFKA_TOPIC_WIND = "wind-stations"

# Initialize Kafka Producer
try:
    producer_wind = KafkaProducer(
    ...
    st.sidebar.warning(f"⚠ Kafka not available: {e}")
...
def append_to_csv(data_dict):
    """Append data to CSV file and send to Kafka"""
    ...
    try:
        ...
        # Send to Kafka
        if kafka_available and producer_wind:
            try:
                # Send message to Kafka
                future = producer_wind.send(
                    KAFKA_TOPIC_WIND,
                    value=data_dict,
                    key=data_dict['station_id'].encode('utf-8')  # Use station_id as key for partitioning
                )
                
                # Optional: Wait for confirmation (can be removed for better performance)
                record_metadata = future.get(timeout=10)
                
                # Uncomment to see Kafka publish details
                # st.sidebar.success(f"✅ Sent to Kafka: {record_metadata.topic}[{record_metadata.partition}]")
...
def generate_real_time_prediction(api_data, station_id):
    ...
    base_wind_speed = api_data["wind_speed_mps"]
    wind_noise = random.uniform(-0.3, 0.3)
    predicted_wind_speed = max(0.1, base_wind_speed + wind_noise)
    
    # Recalculate ALL parameters using the predicted wind speed
    new_predictions = calculate_power_predictions(
        predicted_wind_speed,
        api_data["wind_dir_deg"],
        api_data["air_temperature_C"],
        api_data["air_pressure_hPa"], 
        api_data["humidity_percent"],
...
if should_update:
    for station in stations:
        ...
        if not api_data.empty:
            ...
            # Log real-time prediction to CSV and Kafka
...
st.sidebar.subheader("📨 Kafka Status")
if kafka_available:
    st.sidebar.success("✅ Connected to Kafka")
    st.sidebar.write(f"Brokers: {KAFKA_BOOTSTRAP_SERVERS}")
    st.sidebar.write(f"Topic: {KAFKA_TOPIC_WIND}")
else:
    st.sidebar.error("❌ Kafka not available")
    st.sidebar.write("Data will be logged to CSV only")
...
st.sidebar.write("- Streams to Kafka" if kafka_available else "- CSV only (Kafka unavailable)")
